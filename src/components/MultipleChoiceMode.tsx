...
export default function MultipleChoiceMode({ set, settings, onEnd, onExit }: MultipleChoiceModeProps) {
  ...
  const [mistakes, setMistakes] = useState<VocabPair[]>([])
    ...
    async function fetchWords() {
      ...
      const optionCandidates = (() => {
        ...
        if (settings.direction === 'reverse') {
          pool = fullWords.map(w => ({ ...w, word_1: w.word_2, word_2: w.word_1 }))
        } else if (settings.direction === 'both') {
          const forward = fullWords
          const reverse = fullWords.map(w => ({ ...w, word_1: w.word_2, word_2: w.word_1 }))
          pool = [...forward, ...reverse]
        }
        const opts = pool.map(w => w.word_2).filter(Boolean)
  ...
  async function generateQuestion(currentWord: VocabPair) {
    const correctAnswer = currentWord.word_2
        ...
        <div className="max-w-6xl mx-auto w-full flex flex-col flex-1 overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-6 flex-shrink-0" style={{ animation: 'slideInDown 0.4s ease-out' }}>
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-semibold text-sm transition-all duration-300 flex items-center gap-2 flex-shrink-0 ${layoutMode === 'vertical'
