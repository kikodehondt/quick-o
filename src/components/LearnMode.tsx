...
interface LearnProgressState {
  mode: 'learn'
  activeWords: VocabPair[]
  masteredWords: VocabPair[]
  currentIndex: number
  correctCount: number
...
export default function LearnMode({ set, settings: initialSettings, onEnd }: LearnModeProps) {
  ...
  const [allWords, setAllWords] = useState<VocabPair[]>([])
  const [totalUniqueWords, setTotalUniqueWords] = useState(0)
  const [activeWords, setActiveWords] = useState<VocabPair[]>([])
  const [masteredWords, setMasteredWords] = useState<VocabPair[]>([])
  ...
  useEffect(() => {
    if (!showFeedback && inputRef.current) inputRef.current.focus()
  ...
  async function fetchWordsWithSettings(currentSettings: StudySettings) {
    try {
      // Fetch all words using pagination via set_pairs
      ...
      if (currentSettings.direction === 'both') {
        ...
        const reverse = processed.map(w => ({ ...w, word_1: w.word_2, word_2: w.word_1 }))
        processed = [...forward, ...reverse]
      } else if (currentSettings.direction === 'reverse') {
        processed = processed.map(w => ({ ...w, word_1: w.word_2, word_2: w.word_1 }))
      }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const correct = checkAnswer(userAnswer, currentWord.word_2, settings.caseSensitive, settings.accentSensitive)
  ...
  function nextWord(forceAdvance?: boolean) {
    ...
    // else: blijven op hetzelfde woord tot correct is
    setShowFeedback(false)
    setUserAnswer('')
    setShowHint(false)
  }
  ...
  function getHint() {
    ...
    const answer = activeWords[currentIndex].word_2
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl p-12 card-shadow text-center max-w-md">
          ...
          <button onClick={onEnd} className="btn-gradient text-white px-6 py-3 rounded-xl font-semibold">Terug</button>
        </div>
      </div>
    )
  }
        ...
        <div className="bg-white rounded-3xl p-12 card-shadow text-center max-w-lg">
          ...
          <div className="flex justify-center gap-8 mb-8">
            <div className="text-center">
              <div className="flex items-center justify-center gap-2 mb-2"><CheckCircle className="w-6 h-6 text-green-500" /><span className="text-3xl font-bold text-green-600">{correctCount}</span></div>
              <p className="text-sm text-gray-600">Correct</p>
            </div>
            <div className="text-center">
              <div className="flex items-center justify-center gap-2 mb-2"><XCircle className="w-6 h-6 text-red-500" /><span className="text-3xl font-bold text-red-600">{incorrectCount}</span></div>
              <p className="text-sm text-gray-600">Herhaald</p>
            </div>
          </div>
          <div className="flex gap-4">
            <button onClick={restart} className="flex-1 btn-gradient text-white px-6 py-3 rounded-xl font-semibold hover:opacity-90 transition-opacity flex items-center justify-center gap-2"><RotateCcw className="w-5 h-5" />Opnieuw</button>
            <button onClick={onEnd} className="flex-1 bg-gray-100 text-gray-700 px-6 py-3 rounded-xl font-semibold hover:bg-gray-200 transition-colors flex items-center justify-center gap-2"><ArrowLeft className="w-5 h-5" />Terug</button>
          </div>
        </div>
      </div>
    )
  ...
  const similarity = userAnswer
    ? calculateSimilarity(userAnswer, currentWord.word_2, {
    ...
    <div className="min-h-screen flex flex-col p-4 md:p-8 relative overflow-hidden" style={{ background: 'linear-gradient(-45deg, #10b981 0%, #059669 25%, #047857 50%, #065f46 75%, #10b981 100%)', backgroundSize: '400% 400%', animation: 'gradientShift 20s ease infinite' }}>
      ...
      <div className="max-w-4xl mx-auto w-full flex-1 flex flex-col">
        ...
        <div className="w-full bg-white/20 rounded-full h-4 mb-6">
          <div className="bg-gradient-to-r from-green-400 to-green-600 h-4 rounded-full transition-all duration-500 flex items-center justify-end pr-2" style={{ width: `${progress}%` }}>
            {progress > 10 && (<span className="text-white text-xs font-bold">{Math.round(progress)}%</span>)}
          </div>
        </div>
        <div className="flex justify-center gap-6 mb-8">
          <div className="flex items-center gap-2 bg-green-500/20 text-white px-4 py-2 rounded-xl"><CheckCircle className="w-5 h-5" /><span className="font-bold">{correctCount}</span></div>
          <div className="flex items-center gap-2 bg-orange-500/20 text-white px-4 py-2 rounded-xl"><RotateCcw className="w-5 h-5" /><span className="font-bold">{incorrectCount}</span></div>
        </div>
        <div className="flex-1 flex items-center justify-center mb-8">
          <div className="bg-white rounded-3xl p-12 card-shadow w-full max-w-2xl">
            <div className="text-center mb-8">
              <p className="text-sm font-semibold text-gray-500 mb-4 uppercase tracking-wide">Vertaal naar {settings.direction === 'reverse' ? set.language1 : set.language2}</p>
              <p className="text-4xl md:text-5xl font-bold text-gray-800 mb-6">{currentWord.word_1}</p>
              {!showFeedback && (
                <form onSubmit={handleSubmit}>
                  <input ref={inputRef} type="text" value={userAnswer} onChange={(e) => setUserAnswer(e.target.value)} className="w-full px-6 py-4 text-2xl text-center rounded-xl border-2 border-gray-300 bg-white text-gray-900 placeholder:text-gray-400 focus:border-green-500 focus:ring-2 focus:ring-green-200 focus:outline-none transition-all mb-4" placeholder="Type je antwoord..." autoComplete="off" autoFocus />
                  {userAnswer && similarity > 0 && (
                    <div className="mb-4">
                      <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                        <div className={`h-2 transition-all duration-300 ${similarity >= 80 ? 'bg-green-500' : similarity >= 50 ? 'bg-yellow-500' : 'bg-red-500'}`} style={{ width: `${similarity}%` }} />
                      </div>
                    </div>
                  )}
                  {showHint && (
                  ...
                  <div className="flex gap-3">
                    <button type="submit" disabled={!userAnswer.trim()} className="flex-1 btn-gradient text-white px-6 py-3 rounded-xl font-semibold hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed">Controleer</button>
                    {!showHint && (<button type="button" onClick={() => setShowHint(true)} className="bg-blue-100 text-blue-600 px-6 py-3 rounded-xl font-semibold hover:bg-blue-200 transition-colors flex items-center gap-2"><AlertCircle className="w-5 h-5" />Hint</button>)}
                  </div>
                </form>
              )}
              {showFeedback && !isCorrect && (
                <div className="rounded-2xl p-8 bg-red-50">
                  <div className="flex items-center justify-center mb-4"><XCircle className="w-16 h-16 text-red-500" /></div>
                  ...
                  <div className="mb-6">
                    ...
                    <p className="text-3xl font-bold text-gray-800">{currentWord.word_2}</p>
                    <p className="text-gray-600 mt-2">Jouw antwoord was:</p>
                    <p className="text-xl text-gray-600">{userAnswer}</p>
                  </div>
                  <button onClick={() => nextWord()} className="btn-gradient text-white px-8 py-3 rounded-xl font-semibold hover:opacity-90 transition-opacity">Volgende</button>
                </div>
              )}
            </div>
          </div>
