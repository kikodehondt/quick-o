...
function App() {
  ...
  const [totalSetCount, setTotalSetCount] = useState<number>(0)
  ...
  // Fetch total set count (server side, including private ones)
  useEffect(() => {
    const fetchTotalCount = async () => {
      try {
        const { data, error } = await supabase.rpc('get_total_sets')
        if (!error && typeof data === 'number') {
          setTotalSetCount(data)
        } else {
          // Fallback if RPC fails or doesn't exist yet
          console.warn('Could not fetch total sets count via RPC, falling back to loaded sets length')
        }
      } catch (err) {
    ...
    fetchTotalCount()
  }, [sets]) // Re-fetch when sets change (e.g. create/delete)
  ...
  const handleDeleteSet = useCallback(async (setId: number) => {
    try {
      // Deleting a set automatically cascades to set_pairs and progress due to foreign keys.
      // We don't delete vocab_pairs to preserve shared words history.
          ...
          <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center justify-center gap-3 sm:gap-4 text-white/80 mt-6 w-full">
            ...
            <div className="flex gap-2 w-full sm:w-auto justify-center">
              <div className="flex items-center gap-1 sm:gap-2 px-3 py-2 rounded-xl bg-white/10 border border-white/10 hover:bg-white/20 transition-all duration-300 transform hover:scale-110 flex-1 sm:flex-none justify-center" style={{ animation: 'slideInLeft 0.6s ease-out', animationDelay: '0.1s' }} >
                ...
                <span className="text-sm sm:text-base">{totalSetCount || sets.length} Sets</span>
              </div>
